...
modeldat_openforest <- read_csv("data/modeldat_openforest.csv")
modeldat_openforest$rate <- modeldat_openforest$disturbance_ha / modeldat_openforest$openforest_ha
modeldat_openforest$rate_model <- round(modeldat_openforest$disturbance_ha, 0) / modeldat_openforest$openforest_ha
modeldat_openforest$cluster <- factor(modeldat_openforest$cluster)
modeldat_openforest$forest <- as.integer(modeldat_openforest$openforest_ha)

modeldat <- read_csv("data/modeldat.csv") %>%
  dplyr::select(site, year, rate10, rate50, disturbance, forestcover_10, forestcover_50)

no_open_forest <- modeldat_openforest %>%
  group_by(site) %>%
  summarise(ncf = sum(openforest_ha) == 0) %>%
  filter(ncf == TRUE)

modeldat_openforest <- modeldat_openforest %>%
  filter(!(site %in% no_open_forest$site))

# Compare rates -----------------------------------------------------------

modeldat_compare <- modeldat_openforest %>%
  left_join(modeldat)

ggplot(modeldat_compare, aes(x = rate, y = rate10, col = cluster)) +
  geom_point() +
  geom_abline(intercept = 0, slope = 1) +
  labs(x = "Open forest rate", y = "All forest rate") +
  facet_wrap(~biome)
...
write_csv(parameters$mu %>% data.frame(mu = .) %>% 
            rownames_to_column(var = "predictor"), paste0("results/openforest/disturbance_rate_model_model_parameters_mu_openforest.csv"))
write.csv(parameters$Sigma %>% as.matrix(.) %>% 
            as.data.frame(), paste0("results/openforest/disturbance_rate_model_parameters_Sigma_openforest.csv"))
write_csv(parameters$sigma_year %>% 
            data.frame(sigma_year = .), paste0("results/openforest/disturbance_rate_model_parameters_sigma_year_openforest.csv"))
write.csv(parameters$sigma_sites %>% 
            data.frame(sigma_site = .), paste0("results/openforest/disturbance_rate_model_parameters_sigma_site_openforest.csv"))
...
write_csv(pred_summary_openforest, paste0("results/openforest/disturbance_rates_summary_openforest.csv"))
...
p <- ggplot(sim) + 
  ...
  geom_vline(data = pred_summary_openforest, aes(xintercept = mean), linetype = "dashed") +
  geom_text(data = pred_summary_openforest, aes(x = mean * 1, 
                                     y = c(3000, 1000, 100), 
                                     label = paste0(round(mean * 100, 2), " % per yr.")),
            hjust = -0.15)

ggsave("results/openforest/disturbance_rates_openforest.pdf", p, width = 7.5, height = 2.5)
