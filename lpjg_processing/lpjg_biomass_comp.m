...
lpjg_base_file='/Users/pughtam/LPJG/disturbance_prognostic_runs/100_flat_orig_r5511/postproc/cpool_2001_2014';
lpjg_sens_file='/Users/pughtam/LPJG/disturbance_prognostic_runs/simplemodel_best_est_100patch/cpool_2001_2014';
...
cveg_base_lpjg_in=lpj_to_grid_func_centre([datafol,lpjg_base_file],1,0);
cveg_base_lpjg=squeeze(cveg_base_lpjg_in(:,:,:,1));

cveg_sens_lpjg_in=lpj_to_grid_func_centre([datafol,lpjg_sens_file],1,0);
cveg_sens_lpjg=squeeze(cveg_sens_lpjg_in(:,:,:,1));

cveg_base_lpjg(cveg_base_lpjg==0)=NaN;
cveg_sens_lpjg(cveg_sens_lpjg==0)=NaN;
...
if use_fmask
    ...
    fmask=ncread('/data/Disturbance/hansen_forested_frac_05.nc4','forested_50_percent'); %Percentage of gridcell coverage with at least 50% forest.
    fmask=(double(fmask)/100)'; %Convert to fraction
    fmask(fmask<0.25)=NaN;
    
    cveg_05(isnan(fmask)==1)=NaN;
    cveg_base_lpjg(isnan(fmask)==1)=NaN;
    cveg_sens_lpjg(isnan(fmask)==1)=NaN;
end
cveg_avt_05=cveg_05;
...
aa=find(isnan(cveg_avt_05(:))==1 | isnan(cveg_base_lpjg(:))==1 | isnan(cveg_sens_lpjg(:))==1);
cveg_avt_05(aa)=[];
cveg_base_lpjg(aa)=[];
cveg_sens_lpjg(aa)=[];
clear aa
...
subplot(1,2,2)
hold on
[N_sens,C]=hist3([cveg_avt_05,cveg_sens_lpjg],[100,100]);
    p1=pcolor(C{1},C{2},N_sens'); set(p1,'linestyle','none')
plot(1:50,1:50,'k')
set(gca,'XLim',[0 50],'YLim',[0 50])
mdl2=fitlm(cveg_avt_05,cveg_sens_lpjg,'Intercept',true);
ypred_sens=predict(mdl2,cveg_avt_05);
plot(cveg_avt_05,ypred_sens,'r')
rmse_sens=sqrt(mean((ypred_ideal-cveg_sens_lpjg).^2));
text(1,47,'(b)')
text(1,42,['R^{2}=',mat2str(round(mdl2.Rsquared.Ordinary,3))])
text(1,37,['RMSE=',mat2str(round(rmse_sens,3))])
title('\Modified LPJ-GUESS')
xlabel('Observed biomass (kg C m^{-2})')
